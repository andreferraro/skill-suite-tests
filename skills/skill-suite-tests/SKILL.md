...
7. **Projetar o conjunto mínimo.** Montar a rastreabilidade `risco -> mecanismo protetivo -> estado ou fase -> cenário -> oráculo`. Para cada cenário, definir precondições, estímulo, dados, isolamento, cleanup e evidência esperada. Rastrear guards, validações, locks, transações, deduplicação, ordering, retry e mecanismos equivalentes encontrados no alvo para um cenário sensível ou justificar por que estão fora do recorte. Usar `protective_guards`, `states` e `lifecycle_actions` do analisador como checklist confirmável. Para cancelamento, reset, timeout, retry, desconexão e unmount, enumerar cada estado ativo relevante. Criar um cenário por estado quando a proteção ou o efeito observável mudar. Não tratar um cenário representativo como cobertura de todo o ciclo de vida.
...
9. **Provar sensibilidade.** Partir de uma execução verde. Para cada mecanismo protetivo de alto impacto coberto, executar uma mutação local temporária que remova ou enfraqueça esse mecanismo, exigir que o cenário rastreado falhe, restaurar o arquivo byte a byte e executar verde novamente. Se a remoção ou o weakening exato sobreviver, corrigir o cenário; uma inversão que falha não substitui essa prova. Proteções distintas para estados ou fases distintas exigem provas distintas. Não usar uma única mutação como evidência para riscos, estados ou mecanismos que ela não exercita. "Não altere código de produção" impede mudança no resultado entregue e continua permitindo essa prova transitória restaurada; deixar de mutar somente quando o usuário proibir explicitamente mutações temporárias ou quando elas forem inseguras. Nesses casos, registrar o motivo e executar um controle negativo equivalente. Uma descrição de que o teste falharia não é evidência. Quando ajudar, usar `python <skill-root>/scripts/prove_test_sensitivity.py --root <projeto> --file <arquivo> --search <texto-exato> --replace <mutação> [--occurrence <n>] -- <comando-do-teste>`; usar `--occurrence` para provar separadamente guards repetidos identificados pelo analisador. O helper não usa shell e restaura o arquivo mesmo após erro.
...
- Para cancelamento ou reset, exercitar separadamente cada fase alcançável, como antes do callback assíncrono, durante uma operação pendente e entre lotes. Primeiro executar a ação e aguardar o estado pós-cancelamento observável. Depois, em um ciclo assíncrono separado, liberar e aguardar cada callback ou Promise obsoleta. Só então afirmar que o estado permaneceu cancelado e que nenhum novo efeito ocorreu. Em React, separar essas etapas com `await act(async () => ...)` ou interação equivalente do harness; disparar `reset` e callback obsoleto no mesmo ciclo pode esconder a regressão pelo batching de estado.
- Em callbacks obsoletos, usar payload não vazio capaz de produzir um efeito proibido se o guard falhar, como iniciar um lote, persistir um item ou emitir um evento. Payload vazio não prova supressão quando o fluxo defeituoso também termina sem efeito externo.
- Distinguir falha fatal do parser ou transporte de erros parciais retornados junto aos dados. Quando a implementação trata ambos, criar oráculos separados para o callback fatal e para `results.errors`, itens rejeitados ou mecanismo equivalente.
...
- rastreabilidade entre risco, mecanismo, estado ou fase, cenário e oráculo;
